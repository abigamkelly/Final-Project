...
def evaluate_model(true_labels, predicted_labels, confidence_scores=None):
    ...
    avg_confidence = None  # Since we are not using confidence scores now
...
def main():
    ...
    EXPERT_DEPLOYMENTS = []  # e.g., ["gpt-4o-mini", "gpt-4o-2", "gpt-4o-2"]
    EXPERT_THRESHOLDS = []   # e.g., [0.85, 0.88, 0.9]
...
Example for an emergency tweet:
1
0.95
This tweet reports a severe airplane accident causing multiple fatalities, indicating a real emergency.

Example for a non-emergency tweet:
0
0.9
This tweet uses disaster-related terminology metaphorically in a casual context without referring to any actual emergency.
'''
)
